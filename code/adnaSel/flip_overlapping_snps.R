...
ukbb <- ukbb %>% separate(ID, c("tmp", "POS"), remove = FALSE) %>% filter(ukbb_wbs$ALT_FREQS > 0.01 & ukbb_wbs$ALT_FREQS < 0.99) %>% select("#CHROM",	"ID","POS","REF",	"ALT")
...
fwrite(df_EN, paste0(r_outpath,"EUROPEAN_NEOLITHIC_chr", chr_num, ".rvec"), row.names = F, col.names = T, quote = F, sep = "\t")

df_BA <- df_BA %>% select("CHROM", "ID", "REF", "ALT", "ss")
colnames(df_BA) <- c("CHROM", "ID", "REF", "ALT", "r")
fwrite(df_BA, paste0(r_outpath,"BRONZE_AGE_chr", chr_num, ".rvec"), row.names = F, col.names = T, quote = F, sep = "\t")
...
dfSNP <- dfEN %>% select("ID")
fwrite(dfSNP, snp_outfile, row.names = F, col.names = T, quote = F, sep = "\t")
