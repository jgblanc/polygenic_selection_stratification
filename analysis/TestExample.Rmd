...
```{r}
co <- fread("~/Downloads/chr-all_test-all_Afr-Eur.txt")

ggplot(co, aes(x = FGr, y = FGr, color = POP)) + geom_point()

plot(df$FGr, df$PC1)
cor(df$FGr, df$PC1)

df %>% filter(IID == "4513184")
```
```{r}
df <- fread("~/Downloads/none.txt")

ggplot(df, aes(x = FGr, y = PC1, color = POP)) + geom_point()
