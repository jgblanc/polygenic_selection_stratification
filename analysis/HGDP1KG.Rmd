...
#dfPC <- dfPC %>% filter(contrasts == "eurasia-lat" | contrasts == "eurasia-long")
```
...
```{r}
dfPlot <- dfPC %>% filter(gwas_type == "all" & test_type == "all", L == "pruneall", contrasts == "afr-amr") %>% group_by(gwas) %>% mutate(cumVarExp = cumsum(VarExp)) %>% ungroup()
...
```{r}
pl <- dfOverlap %>% filter(L == "pruneall") %>% ggplot(aes(x = contrasts, y = error, color = gwas_type2)) + geom_point() + facet_wrap(~gwas, nrow =2) + theme_bw(base_size = 14) + theme(plot.title = element_text(hjust = 0.5), axis.text.x = element_text(size = 10,angle = 45, hjust = 1)) + scale_color_manual(values = c("purple4", "lightseagreen"))  + guides(color ="none") + xlab("")
pl
ggsave("~/Desktop/tmp.png",pl,  height = 5)
```
...
```{r}
pl <- dfPC %>% filter(gwas_type == "even" & test_type == "odd" & L == "pruneall" & PC <= 10)%>% ggplot(aes(x = PC, y = B2)) + geom_bar(stat = "identity", fill = "darkgreen", color = "darkgreen") + facet_grid(vars(gwas), vars(contrasts)) + theme_bw(base_size = 12) + ylab(TeX("$B_{i}^2$"))  + theme(plot.title = element_text(hjust = 0.5))+ scale_x_continuous(breaks = seq(0, 20, by = 10))
pl
...
```{r}
pl <- dfPC %>% filter(gwas_type == "all" & test_type == "all" & L == "pruneall" & (gwas == "ALL" | gwas == "WBS")) %>% ggplot(aes(x = PC, y = r2)) + geom_bar(stat = "identity", fill = "darkgreen", color = "darkgreen") + facet_grid(vars(gwas), vars(contrasts)) + theme_bw(base_size = 12) + ylab(TeX("$R_{K}^2$")) + geom_hline( aes(yintercept = 1-Error), color = "red", linetype = 2, linewidth = 1) + scale_x_continuous(breaks = seq(0, 40, by = 10)) + theme(axis.text.x = element_text(size = 6), plot.title = element_text(hjust = 0.5)) + ylim(-0.1,1)
pl
...
dfPlot <- dfQ %>% filter(contrasts == "nfe-afr")

ggplot(dfPlot, aes(x = pc, y = q)) + geom_point() + facet_grid(vars(covar), vars(gwas))
```
