...
colnames(dfEur) <- c("IID", "pop", "subpop", "lat", "long")
dfEur <- dfEur %>% mutate(SDI = case_when(subpop == "Sardinian" ~ "sdi", TRUE ~ "eur")) %>% dplyr::select(IID, SDI, everything())
dfEur <- dfEur %>% mutate(FID = 0) %>% select(FID, everything())
...
dfPlot <- dfAllOverlap %>% filter(contrasts %in% c("BronzeAge", "EuropeanNeolithic", "Historical"))
...
for (i in 1:L) {
  G[,i] <- c(rnorm(M, mean = 0, sd = 1))
}
G <- scale(G)
 

# Make r 
r <- c(rnorm(L, mean = 0, sd = 1))
r <- scale(r)
...
```{r}
FGr_hat <- apply(data, 1, sum) * (1/sqrt(L-1)) * (sqrt(M/L))
var(FGr_hat)
D <- t(FGr_hat) %*% FGr_hat *(1 / M)* (1/L) 

D

var(data[,1] * 1/sqrt(2315) * (sqrt(M/L)))
```

```{r}
df <- fread("~/Downloads/eas-nfe_1.FGr.glm.linear")
