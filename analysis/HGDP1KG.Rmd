...
```{r}
df %>% filter(contrasts == "eur-lat" & phenotype == "EosinophillPercentage") %>% ggplot(aes(x = gwas, y = q, color = as.character(pc))) + geom_point() 
```
...
df %>% filter(contrasts == "nfe-amr" & phenotype == "Glucose") %>% ggplot(aes(x = gwas, y = q, color = as.character(pc))) + geom_point() 
```
