...
dfEurAsia <- dfEurAsia %>% select("IID", "latitude", "longitude", "project_meta.project_pop", "population")
colnames(dfEurAsia) <- c("#IID", "lat", "long", "pop", "subpop")
...
dfEur <- dfEur %>% select("IID", "latitude", "longitude", "project_meta.project_pop", "population") %>% 
  filter(population != "CEU")
colnames(dfEur) <- c("#IID", "lat", "long", "pop", "subpop")
...
dfEur <- dfEur %>% select("IID", "latitude", "longitude", "project_meta.project_pop", "population") 
colnames(dfEur) <- c("#IID", "lat", "long", "pop", "subpop")
dfEur <- dfEur %>% mutate(SDI = case_when(subpop == "Sardinian" ~ "sdi", TRUE ~ "eur"))
...
```{r}
dfAll <- dfFilter %>%  filter(project_meta.project_pop %in% c("nfe", "afr", "amr", "eas", "sas")) %>% select("IID", "latitude", "longitude", "project_meta.project_pop", "population")
colnames(dfAll) <- c("#IID", "lat", "long", "pop", "subpop")
